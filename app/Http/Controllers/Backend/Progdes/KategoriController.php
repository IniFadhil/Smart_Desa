...
class KategoriController extends Controller
{
  ...
  {
      $this->middleware('permissions:kategori-program');
  }
  ...
  public function index()
  {
      try{
          ...
          return view('backend.program.kategori.list', $data);
      }catch(\Exception $e){
  ...
  {
      try{
          return view('backend.program.kategori.create');
      }catch(\Exception $e){
          toastr()->error($e->getMessage(),'Gagal');
          return back();
      }
  ...
  public function createProccess(Request $request)
  {
      try{
          ...
          $data['id'] = $this->generateAutoNumber('ds_program_kategori');
          $data['created_by'] = Auth::user()->name;
          $data['desa_id'] = empty(Auth::user()->desa_id)?Session::get('desa_id'):Auth::user()->desa_id;

          $program = ProgramKategori::create($data);
          toastr()->success('Data Berhasil Ditambahkan','Sukses');
          return redirect()->route('backend.program.category');
  ...
  public function edit($id)
  {
      try{
          ...
          $data['kategori'] = ProgramKategori::find($id);
          return view('backend.program.kategori.edit',$data);
      }catch(\Exception $e){
          toastr()->error($e->getMessage(),'Gagal');
          return back();
      }
  ...
  public function editProccess(Request $request,$id)
  {
      try{
          ...
          $kategori = ProgramKategori::find($id);
          $kategori->update($data);
          toastr()->success('Data Berhasil Diubah','Sukses');
          return redirect()->route('backend.program.category.detail',['id'=>$kategori->encodeHash($kategori->id)]);
      }catch(\QueryBuilder $e){
          toastr()->error($e->getMessage(),'Gagal');
          return back();
      }
  ...
  public function detail($id)
  {
      try{
          $id = $this->decodeHash($id);
          $data['kategori'] = ProgramKategori::find($id);
          return view('backend.program.kategori.detail',$data);
      }catch(\Exception $e){
          toastr()->error($e->getMessage(),'Gagal');
          return back();
      }
  ...
  {
      $rules = [
          'name' => 'required|max:191|unique:ds_program_kategori,name,'.$request->id,
  ...
  {
      if(!empty($request->id)){
          $kategori = ProgramKategori::find($request->id);
      }
