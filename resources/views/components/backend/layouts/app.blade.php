        ...
        <aside
            ...
            <div class="mb-10 text-center">
                <a href="{{ route('backend.dashboard') }}">
                    <h1 class="text-2xl font-bold text-gray-800">SmartDesa Panel</h1>
                </a>
            </div>

            <nav>
                <ul class="flex flex-col space-y-2">
                    {{-- Link Dashboard Statis --}}
                    <li>
                        <a href="{{ route('backend.dashboard') }}"
                    ...
                    @if (isset($moduls))
                        @foreach ($moduls as $modul)
                            @php
                                // Filter submenu yang valid untuk modul ini
                                $submenus = $permissions->filter(function ($permission) use ($modul) {
                                    return $permission->modul_id == $modul->id && $permission->menu;
                                });
                                // dd($submenus[0]);
                            @endphp

                            {{-- Hanya tampilkan dropdown jika ada minimal satu submenu --}}
                                    ...
                                    <div x-data="{ open: false }">
                                        ...
                                        <div x-show="open" x-transition class="mt-1 pl-8 space-y-1">
                                            <ul class="flex flex-col space-y-1">
                                                @foreach ($submenus as $permission)
                                                    {{-- {{ dd($permission->menu->url) }} --}}
                                                    <li>
                                                        {{-- @if (!empty($permission->menu->url) && Route::has($permission->menu->url)) --}}
                                                        <a href="{{ route($permission->menu->route) }}"
                                                            {{-- <a href="#" --}}
