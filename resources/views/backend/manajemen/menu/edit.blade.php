    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white shadow-md rounded-lg p-8">
            <div class="flex justify-between items-center mb-6 border-b pb-4">
                <h1 class="text-2xl font-semibold text-gray-700">Edit Menu: {{ $menu->name }}</h1>
                <a href="{{ route('backend.manajemen.menu.index') }}"
                    class="text-sm text-gray-600 hover:text-gray-900">&larr; Kembali ke Daftar</a>
            </div>

            <form action="{{ route('backend.manajemen.menu.update', $menu) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="mb-4 col-span-2">
                        ...
                        <select name="status" id="status"
                            ...
                            <option value="1" @selected(old('status', $menu->status) == 1)>Aktif</option>
                            <option value="0" @selected(old('status', $menu->status) == 0)>Tidak Aktif</option>
                        </select>
                    </div>
                </div>
