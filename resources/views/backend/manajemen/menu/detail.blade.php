<x-backend.layouts.app>
    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">Detail Menu</h2>
        </div>

        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-y-4">
                <div class="text-sm font-medium text-gray-500">Nama</div>
                <div class="text-sm text-gray-900 col-span-3">{{ $menu->name ?? '-' }}</div>

                <div class="text-sm font-medium text-gray-500">Icon</div>
                <div class="text-sm text-gray-900 col-span-3"><i class="{{ $menu->icon }}"></i>
                    ({{ $menu->icon ?? '-' }})</div>

                <div class="text-sm font-medium text-gray-500">URL / Route Name</div>
                <div class="text-sm text-gray-900 col-span-3">{{ $menu->url ?? '-' }}</div>

                <div class="text-sm font-medium text-gray-500">Menu Induk</div>
                <div class="text-sm text-gray-900 col-span-3">{{ $menu->parent->name ?? 'Menu Utama' }}</div>

                <div class="text-sm font-medium text-gray-500">Urutan</div>
                <div class="text-sm text-gray-900 col-span-3">{{ $menu->order ?? '-' }}</div>
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end space-x-4">
            ...
            <a href="{{ route('backend.manajemen.menu.edit', $menu) }}"
