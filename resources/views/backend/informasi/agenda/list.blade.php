<x-backend.layouts.app>
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold text-gray-700">Daftar Agenda</h1>
        <a href="{{ route('backend.informasi.agenda.create') }}"
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($agendas as $agenda)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">{{ Str::limit($agenda->title, 40) }}</td>
                            <td class="px-6 py-4">
                                {{ \Carbon\Carbon::parse($agenda->start_date)->translatedFormat('d F Y, H:i') }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <div class="flex justify-center items-center space-x-4">
                                    <a href="{{ route('backend.informasi.agenda.show', $agenda) }}"
                                    ...
                                    <form action="{{ route('backend.informasi.agenda.toggleStatus', $agenda) }}"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.toggle-status-form').forEach(form => {
                form.addEventListener('submit', function(event) {
                    ...
                    Swal.fire({
                        title: 'Konfirmasi',
                        text: `Apakah Anda yakin ingin ${buttonTitle} agenda ini?`,
                        icon: 'warning',
