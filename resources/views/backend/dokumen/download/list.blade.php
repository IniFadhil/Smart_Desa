                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="card-header-action">
                            ...
                            <a href="{{route('backend.dokumen.download.create')}}" class="btn btn-success btn-icon"><i
                            ...
                            <table class="table table-striped table-1">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($row->file)
                                            <a href="{{asset('backend/files/download/'.$row->file)}}" target="_blank">{{$row->file}}
                                            </a>
                                            @else
                                            -
                                            @endif
                                        ...
                                        <td>
                                            <span
                                                class="badge badge-{{($row->status == 'show')?'success':'danger'}}">{{($row->status=='show')?'Aktif':'Tidak Aktif'}}</span>
                                        </td>
                                        <td>
                                            @if(Session::get('permission')->update == 1)
                                            <a href="{{route('backend.dokumen.download.edit',['id' => $row->encodeHash($row->id)])}}"
                                            ...
                                            <a href="{{route('backend.dokumen.download.detail',['id' => $row->encodeHash($row->id)])}}"
...
<form action="{{route('backend.dokumen.download.active')}}" method="post">
    {{csrf_field()}}
...
<form action="{{route('backend.dokumen.download.inactive')}}" method="post">
    {{csrf_field()}}
    <input type="hidden" class="id" value="" name="id">
...
<!-- <form action="{{route('backend.dokumen.download')}}" method="post" enctype="multipart/form-data">
    {{csrf_field()}}
